...
void solve()
{
    ...
    while(!q.empty()) {
        ...
        if(v[i][j] != '@') continue;
        if(!check(i, j, n, m, v)) continue;

        v[i][j] = 'x';  // mark removed (not '@', so won't count in neighbors)
        ans++;

        // Re-check all 8 neighbors — removal may have freed them
        for(auto [di, dj] : dir8) {
            int ni = i + di, nj = j + dj;
            if(ni < 0 || nj < 0 || ni >= n || nj >= m) continue;
            if(v[ni][nj] == '@' && check(ni, nj, n, m, v))
                q.push({ni, nj});
        }
    }
